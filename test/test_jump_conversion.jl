@testset "JuMP-style constraint conversion" begin
    ...
    c1 = add_linear_constraint!(dm, y[1] + 2.0 * y[2] <= 3.0)

    @test c1.a == [1.0, 2.0, 0.0]
    @test c1.sense == :<=
    @test c1.b == 3.0

    c2 = add_linear_constraint!(dm, y[1] - y[3] >= 0.5)

    @test c2.a == [1.0, 0.0, -1.0]
    @test c2.sense == :>=
    @test c2.b == 0.5

    c3 = add_linear_constraint!(dm, y[1] + y[2] + y[3] == 1.0)

    @test c3.a == [1.0, 1.0, 1.0]
    @test c3.sense == :(==)
    @test c3.b == 1.0

    c4 = add_linear_constraint!(dm, sum(y[i] for i in 1:3) <= 2.0)
...
@testset "JuMP-style constraints with constants" begin
    ...
    c = add_linear_constraint!(dm, y[1] + y[2] + 2.0 <= 5.0)
